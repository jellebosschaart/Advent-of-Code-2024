...
def find_touching_cells(grid, x, y, plant):
    ...
    return list(visited), touches - 4*len(list(visited))
...
while len(coords_left) != 0:
    ...
    touching_cells, non_touches = find_touching_cells(data_array, x, y, plant)
    
    print(plant, non_touches)

    area = len(touching_cells)
    
    price += 4*(area**2) - (non_touches)*area
